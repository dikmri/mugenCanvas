...
/// Draw a stroke segment onto a tile's RGBA pixel buffer.
/// `tile_ox`, `tile_oy` are the tile's world-space origin.
/// `anti_alias` controls soft edge blending; false gives a hard pixel edge.
pub fn draw_segment(
    pixels: &mut [u8],
    tile_ox: f32, tile_oy: f32,
    from: (f32, f32), to: (f32, f32),
    size: f32, r: u8, g: u8, b: u8, opacity: f32,
    is_eraser: bool, anti_alias: bool,
) {
    ...
    for i in 0..=steps {
        ...
        draw_circle(pixels, wx - tile_ox, wy - tile_oy, size / 2.0, r, g, b, opacity, is_eraser, anti_alias);
    }
}

fn draw_circle(pixels: &mut [u8], cx: f32, cy: f32, radius: f32, r: u8, g: u8, b: u8, opacity: f32, is_eraser: bool, anti_alias: bool) {
    ...
    for py in y0..=y1 {
        for px in x0..=x1 {
            ...
            let circle_alpha = if anti_alias && d > radius - 1.0 {
                (radius - d).clamp(0.0, 1.0)
            } else {
                1.0
            };
